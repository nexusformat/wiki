...
This definition is mostly written for raw data archive. Simulation and
analysis results may use similar metadata but will probably use a
smaller subset.

One addition necessary for Archived analysis result is links to the
input data. There is currently no scheme for such links in the neXus
definition. I added an 'external\_reference' class that can be replaced
by an official one when defined.
